...
{
    [UpdateModule]
    internal sealed partial class NetworkManager : ModuleImp, INetworkManager
    {
        ...
        {
            foreach (string channelKey in m_NetworkChannels.Keys)
            {
                NetworkChannelBase channelValue = m_NetworkChannels[channelKey];
                channelValue?.Update(elapseSeconds, realElapseSeconds);
            }
        }
        ...
        private void OnNetworkChannelError(NetworkChannelBase networkChannel, NetworkManagerErrorCode errorCode, SocketError socketErrorCode,
            string errorMessage)
        {
